...
namespace UI.Areas.Manage.Controllers
{
    public class InfoController : Controller
    {
        ...
        {
            var result = HttpHelper.HttpGet(Tools.ServerPath + "api/Info/List", "get");
            var data = Newtonsoft.Json.JsonConvert.DeserializeObject<Result<object>>(result);
            ViewData["Info"] = data.Data;
            var res = HttpHelper.HttpGet(Tools.ServerPath + "api/InfoCategory/List", "get");
        ...
        {
            var result = HttpHelper.HttpPost(Tools.ServerPath + "api/Info/Delete/" + id, "post", null);
            var data = Newtonsoft.Json.JsonConvert.DeserializeObject<Result<object>>(result);
            ...
            else
            {
                ...
                if (file.ContentLength <= 1024 * 1024 * 20)
                {
                    ...
                    if (exname == ".jpg" || exname == ".jpeg" || exname == ".png" || exname == ".bmp" || exname == ".gif")
                    {
                        ...
                        //2、保存在本地
                        file.SaveAs(Server.MapPath(Tools.InfoImg + img));
                        return Json(img, JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
        ...
        [HttpPost]
        public ActionResult Add(string title, string img, string content, int categoryID)
        {
            var info = new
            {
                Title = title,
            ...
            var result = HttpHelper.HttpPost(Tools.ServerPath + "api/Info/Add", "post", info);
            var data = Newtonsoft.Json.JsonConvert.DeserializeObject<Result<object>>(result);
        ...
        {
            //获取学生作品详情
            var result = HttpHelper.HttpGet(Tools.ServerPath + "api/Info/FindByID/" + id, "get");
            return Json(result, JsonRequestBehavior.AllowGet);

        }
        [HttpPost]
        public ActionResult Update(int id, string title, string img, string content, int categoryID)
        {
            var info = new
            {
                Title = title,
            ...
            var result = HttpHelper.HttpPost(Tools.ServerPath + "api/Info/Update/" + id, "post", info);
            var data = Newtonsoft.Json.JsonConvert.DeserializeObject<Result<object>>(result);
